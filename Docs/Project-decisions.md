...
✓ The background host rebuilds the browser bundle before serving Dad Radar, writes a bounded local operational log under `runtime/`, and can wait for a manually started server before taking over the configured port.
